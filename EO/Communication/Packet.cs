...
namespace EOHax.EO.Communication
{
	...
	public class Packet
	{
		...
		/// Reads 2 bytes.
		/// </summary>
		public short PeekShort() { return (short)DecodeNumber(data.GetRange(readPos, 2).ToArray()); }
		...
		/// </summary>
		/// <param name="length">Number of bytes to read</param>
		public string PeekFixedString(int length) { return ASCIIEncoding.ASCII.GetString(PeekBytes(length)); }

		/// <summary>
		/// Reads all of the data from the current read position to the end of the string
		/// </summary>
		public string PeekEndString() { return PeekFixedString(Length - readPos); }
		...
		public string PeekBreakString()
		{
			...
			while (breakpos < Length && data[breakpos] != Break)
				++breakpos;
		...
		{
			return ASCIIEncoding.ASCII.GetString(GetBytes(length));
		}
